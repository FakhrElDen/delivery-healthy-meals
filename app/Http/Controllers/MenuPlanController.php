...
use App\Imports\ImportUserMenu;
use Illuminate\Support\Facades\Mail;
use App\Mail\ExcelSheetUploaded;
use App\User;
use App\MenuPlan;
use App\MenuUser;
use App\Status;
...
class MenuPlanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $result = $this->importUserMenu($request);
        if($result){
            $objUser = new User();
            $user=$objUser->getUserById($request['user_id']);
            $data = array(
                'name'    => $user['name'],
                'email'   => $user['email'],
            ); 
            $resultEmail = Mail::to($data['email'])->send(new ExcelSheetUploaded($data));
            return back();
        }
    ...
    function import(Request $request){
        $path1 = $request->file('menu')[0]->store('public/temp');
        $pathSubs = substr($path1, 7);
        $path = storage_path('app').'/'.$path1; 
        $file = $pathSubs;
        $arrMenuPlan = Excel::import(new ImportMenu, $path);
        $objMenuPlan = new MenuPlan();
        $updateMenuPlan = $objMenuPlan->handleMenuTable($request['date'],$request['plan_id'],$file);    
        return true;
    }

    function importUserMenu(Request $request){
        // dd($request->file('menu'));
        $path1 = $request->file('menu')->store('public/temp');
        $pathSubs = substr($path1, 7);
        $path = storage_path('app').'/'.$path1; 
        $file = $pathSubs;
        $arrMenuUser = Excel::import(new ImportUserMenu, $path);
        $objMenuUser = new MenuUser();
        $updateMenuUser = $objMenuUser->handleMenuUserTable($request['user_id'],$file);    
        return true;
    }
